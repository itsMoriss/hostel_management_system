...
# import requests
from .tasks import send_email_task,file_writer,send_username_task,send_email_activation_task,send_activation_change_email_task
...
def send_activation_email(request, email, code):
   ...
   # send_mail(email, 'change_email', context)
    ...
    file_writer.delay(email,my_uri)
    send_email_activation_task.delay(email,subject,my_uri)
...
def send_activation_change_email(request, email, code):
    ...
    subject = context['subject']
    file_writer.delay(email,my_uri)
    send_activation_change_email_task.delay(email,subject,my_uri)
...
def send_reset_password_email(request, email, token, uid):
    ...
    subject = context['subject']
    file_writer.delay(email,my_uri)
    send_email_task.delay(email,subject,my_uri)

def send_forgotten_username_email(email, username):
    context = {
        'subject': _('Your username is'),
        'username': username,
    }
    ...
    # my_uri = context['uri']
    subject = context['subject']
    file_writer.delay(email,username)
    send_username_task.delay(email,subject,username)
